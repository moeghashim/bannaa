...
import { articleContent, siteContent } from "@/lib/content";
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return post ? { title: post.title, description: post.description, alternates: { canonical: `/${locale}/hub/${slug}` }, openGraph: { type: "article", title: post.title, description: post.description, publishedTime: post.date, authors: [post.author] } } : {};
}
export default async function PostPage({ params }: Props) {
  ...
  return <SiteShell content={siteContent[locale]} locale={locale}><article className="blog-article wrap">
    <a href={`/${locale}/hub`}>{articleContent[locale].back}</a>
